...
<x-filament-panels::page>
    ...
    @if ($this->shows('bahan'))
        <x-filament::section class="rekap-card">
            <x-slot name="heading">Pembelian Bahan Baku ({{ $this->materialPurchases->count() }} nota)</x-slot>
            <div style="overflow-x:auto">
                <table class="rekap-table">
                    <thead>
                        <tr><th>Tanggal</th><th>No. Nota</th><th>Vendor</th><th>Dompet</th><th class="num">Total</th></tr>
                    </thead>
                    <tbody>
                        @forelse ($this->materialPurchases as $b)
                            <tr>
                                <td>{{ $b->purchased_at?->format('d/m/Y') }}</td>
                        ...
                        @empty
                            <tr><td colspan="5" class="text-gray-500">Tidak ada pembelian bahan dibukukan pada periode ini.</td></tr>
                        @endforelse
                    </tbody>
                    <tfoot><tr><td colspan="4">Subtotal</td><td class="num">{{ $rp($s['belanja_bahan']) }}</td></tr></tfoot>
                </table>
            </div>
        </x-filament::section>
    @endif

    @if ($this->shows('produksi'))
        <x-filament::section class="rekap-card">
            <x-slot name="heading">Produksi ({{ $this->productions->count() }} nota)</x-slot>
            <x-slot name="description">
                Angka di sini tidak ikut mengurangi laba — belanja bahan sudah menjadi nilai stok,
                dan biaya produksi sudah masuk sebagai modal barang saat terjual.
            </x-slot>
            <div style="overflow-x:auto">
                <table class="rekap-table">
                    <thead>
                        <tr>
                            <th>Tanggal</th><th>No.</th><th>Formula</th>
                            <th class="num">Unit</th><th class="num">Bahan</th><th class="num">Tenaga Kerja</th>
                            <th class="num">Mesin</th><th class="num">Total</th><th class="num">HPP/unit</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($this->productions as $p)
                            <tr>
                                <td>{{ $p->produced_at?->format('d/m/Y') }}</td>
                                <td>{{ $p->production_number }}</td>
                                <td>{{ $p->formula?->name ?: '—' }}</td>
                                <td class="num">{{ rtrim(rtrim((string) $p->output_qty, '0'), '.') }}</td>
                                <td class="num">{{ $rp($p->material_cost) }}</td>
                                <td class="num">{{ $rp($p->service_cost) }}</td>
                                <td class="num">{{ $rp($p->machine_cost) }}</td>
                                <td class="num">{{ $rp($p->total_cost) }}</td>
                                <td class="num">{{ $rp($p->hpp_per_unit) }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="9" class="text-gray-500">Tidak ada produksi dibukukan pada periode ini.</td></tr>
                        @endforelse
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colspan="3">Subtotal</td>
                            <td class="num">{{ rtrim(rtrim((string) $s['unit_produksi'], '0'), '.') }}</td>
                            <td colspan="3"></td>
                            <td class="num">{{ $rp($s['biaya_produksi']) }}</td>
                            <td class="num">{{ $rp($s['hpp_rata']) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
